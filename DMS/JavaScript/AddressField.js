...
jquery_1_11_3_min_p(document).ready(function () {
    ...
    BindFieldAddressGrid(searchtxt);
    BindDataType();
...
////              var rowID = counter + 1;
////        var markup = "<tr><td style='display:none'>" + rowID + "</td><td><input type='checkbox' class='chk_All' id='chk_" +rowID+"'></td><td ><input type='text' class='' id='txtField_" + rowID + "' onkeyup='RemoveClass(this)' autocomplete='off' placeholder='' /></td><td ><input type='text' id='ddlDatatype_" + rowID + "' class=''   autocomplete='off'/></td></tr>";
...
//detailcounter
 ...
 tablename="tbl"+entityfirst[0]+coutryname+"_"+colname;
...
function SaveAddressFields() {
   ...
   var dbcolumns=JSON.stringify(dbfinalcolumnname);
    ...
    type: "POST",
    contentType: "application/json; charset=utf-8",
    url: "../WebServices/Setup.asmx/SaveAddressFields",
    data: "{'JsonFields':'" + JsonFields + "','editFlag':'" + editFlag + "','dbcolumns':" + dbcolumns + ",'entityid':" + entityid + ",'countryid':" + countryid + "}",
    dataType: "json",
    success: function (result) {
            var i = 0;
...
function BindFieldAddressGrid(searchtxt) {
 jquery_1_11_3_min_p("#tblFieldGrid tbody").empty();
    ...
    jquery_1_11_3_min_p.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../WebServices/Setup.asmx/BindFieldAddressGrid",
