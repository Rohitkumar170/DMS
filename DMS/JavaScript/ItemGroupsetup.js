var groupcount = 1; var searchtxt = ''; var ColumnName = []; var editjsondata = ''; var SessionEntityId = 1; var SessionCountryId = 1;
jquery_1_11_3_min_p(document).ready(function () {
 jquery_1_11_3_min_p("#hdnLoad").val(10);
    ...
    jquery_1_11_3_min_p('#btnnew').click(function () {
        jquery_1_11_3_min_p('#itemGroupForm').css('display', 'block');
        jquery_1_11_3_min_p('#itemGroupGrid').css('display', 'none');
...
var counter=0; var i=0;
    jQuery.each(editjsondata.Table2, function (rec) {
        if(editjsondata.Table2[i].CountryId==countryid && editjsondata.Table2[i].EntityId==entityid)
          {
          var rowID = counter + 1;
     var markup = "<tr><td style='opacity: 1;'><input type='checkbox' id='chk_" + rowID + "' class='checkbox'/></td><td ><input type='text' id='txtgroupcode_" + rowID + "' class='fieldName'  autocomplete='off'/></td><td ><input type='text' id='txtgroup_" + rowID + "' onkeyup='Comparevalue(this)' class='fieldName' placeholder='Enter Item Group' autocomplete='off'/></td><td ><input type='text' placeholder='Enter Description' class='fieldName' id='txtgroupname_" + rowID + "' autocomplete='off' /></td><td  style='opacity: 0;'>" + rowID + "</td></tr>";

        jquery_1_11_3_min_p("#tblgroups tbody").append(markup);
        counter = rowID;
         //jquery_1_11_3_min_p('#chkAll').css('display', 'table-cell');
          
         var groupcode="txtgroupcode_"+rowID;
         jquery_1_11_3_min_p('#'+groupcode).val(editjsondata.Table2[i].GroupCode);
         jquery_1_11_3_min_p('#'+groupcode).attr('disabled', 'disabled');
          var gropname="txtgroup_"+rowID;
         jquery_1_11_3_min_p('#'+gropname).val(editjsondata.Table2[i].GroupName);
         jquery_1_11_3_min_p('#'+gropname).attr('disabled', 'disabled');
          var groupdesc="txtgroupname_"+rowID;
...
swal({
title: "Your all data dependency will be lost do you want to proceed?",
text: "",
icon: "warning",
buttons: true,
dangerMode: true,
...
function Deleteentity(country,entity)
{
  jquery_1_11_3_min_p.ajax({
        ...
        url: "../WebServices/ItemSetup.asmx/DeleteEntity",
...
function BindFieldGroupGrid(searchtxt) {
 jquery_1_11_3_min_p("#partnersgrid tbody").empty();
    ...
    jquery_1_11_3_min_p.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../WebServices/ItemSetup.asmx/BindFieldGroupGrid",
...
function BindGroupCode() {
    ...
    var Countryid = SessionCountruId;
    jquery_1_11_3_min_p.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../WebServices/ItemSetup.asmx/BindGroupcode",
...
function FieldgroupAssigned()
{
    ...
    var row = jquery_1_11_3_min_p(this);
    FieldGroup.push({GroupCode: jquery_1_11_3_min_p("#txtgroupcode_" + row.find('td:nth-child(5)').text().trim()).val(), GroupName: jquery_1_11_3_min_p("#txtgroup_" + row.find('td:nth-child(5)').text().trim()).val(), Description: jquery_1_11_3_min_p("#txtgroupname_" + row.find('td:nth-child(5)').text().trim()).val(),Entityid: entityid,Countryid: countryid,CreatedBy: CreatedBy });
     i++;
    });
    ...
    type: "POST",
    contentType: "application/json; charset=utf-8",
    url: "../WebServices/ItemSetup.asmx/SaveGroupFields",
...
function AddRow() {
    ...
    if (jquery_1_11_3_min_p("#txtgroup_" + groupcount).val() != '') {
        var rowID = groupcount + 1;
        var markup = "<tr><td style='opacity: 1;'><input type='checkbox' id='chk_" + rowID + "' class='checkbox'/></td><td ><input type='text' id='txtgroupcode_" + rowID + "' class='fieldName'  autocomplete='off'/></td><td ><input type='text' id='txtgroup_" + rowID + "' onkeyup='Comparevalue(this)' class='fieldName' placeholder='Enter Item Group' autocomplete='off'/></td><td ><input type='text' placeholder='Enter Description' class='fieldName' id='txtgroupname_" + rowID + "' autocomplete='off' /></td><td  style='opacity: 0;'>" + rowID + "</td></tr>";
        jquery_1_11_3_min_p("#tblgroups tbody").append(markup);
        jquery_1_11_3_min_p("#txtgroup_" + rowID).focus();
        groupcount = rowID;
        jquery_1_11_3_min_p("#txtgroup_" + parseInt(groupcount - 1)).attr('disabled', 'disabled');
//        jquery_1_11_3_min_p("#txtgroupname_" + parseInt(groupcount - 1)).attr('disabled', 'disabled');
        jquery_1_11_3_min_p("#txtgroupcode_" + parseInt(groupcount - 1)).attr('disabled', 'disabled');
        jquery_1_11_3_min_p("#txtgroupcode_" + parseInt(groupcount)).attr('disabled', 'disabled');
        var groupcode=jquery_1_11_3_min_p("#txtgroupcode_" + parseInt(groupcount - 1)).val().split('-');
     ...
     if (jquery_1_11_3_min_p("#txtgroup_" + groupcount).val() == '') {
            jquery_1_11_3_min_p("#txtgroup_" + groupcount).addClass("validate");
            jquery_1_11_3_min_p("#txtgroupname_" + groupcount).attr("placeholder", "Enter Item Group!");
        }
...
function Comparevalue(data) {
  ...
  var existfieldid1 = jquery_1_11_3_min_p(table.rows[i].cells[4]).text();
  
   var newid="txtgroup_"+existfieldid1;
    var existfieldid=jquery_1_11_3_min_p('#'+newid).val().trim();
  if(val==existfieldid)
  {
   swal("Group name already exists","Please enter another group!","info")
              .then((value) => {
...
//        type: "POST",
//        contentType: "application/json; charset=utf-8",
//        url: "../WebServices/ItemSetup.asmx/BindEntitydDetails",
//        data: "{}",
//        dataType: "json",
//        async: false,
//        success: function (result) {
